 ...
 * for that statement is reclaimed.
 */
 ...
 * ------------------------------------------------------------
 * Returns the line number of the first line in the program
 * whose number is larger than the specified one.  If no more
 * lines remain, this method returns -1.
 */
...
private:
	...
	SourceLine *header; // Header line of the program (will be pointed to a dummy line)
	SourceLine *cursor; // Keep track of "a" current position

	void recInsertSorted(SourceLine * & program, int lineNb, string source);
	void recDeleteFromList(SourceLine * & program, int lineNumber);
	void positionCursor(int lineNumber);
	void initProgram();
	void deleteProgram();
